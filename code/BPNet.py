...
Because I mainly untilize the Jacobia matrix for mathematical derivation and python coding, which is more concise and much easier, it is recommonded to read the `https://volcano-x.github.io/2024/12/07/bp-network/` firstly before you start to read the python code. 
"""
...
def BPNetTrain(X, H, layerNum, neuronNumList, batchNum, step, actFunction, task):
    ...
    while globalIter < globalIterMax:
        print(
            "{0:-^70}".format(
                "  全局迭代次数:" + str(globalIter) + "/" + str(globalIterMax) + "  "
            ),
            end="\n\n",
        )
...
def showResult(H, Y_hat, task):
    ...
    if task == "classification":
        ...
        print(
            "{0:<26}{1:<0}".format(
                "分类正确率：" + str(correctRate), "损失函数值：" + str(lossFunValue)
            ),
            end="\n\n",
        )
    elif task == "regression":
